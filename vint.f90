...
contains
  ...
  where (ranv(1:n) < 0) ranv(1:n)=ranv(1:n)+2147483579_int32
  iran(1:n)=jran(1:n)
  jran(1:n)=kran(1:n)
  kran(1:n)=ranv(1:n)
  ...
  where (nran(1:n) == 1) nran(1:n)=270369_int32
  ...
  REAL(wp), DIMENSION(MXDIM), SAVE :: dt,dx,x  ,harvest  ! modded
  ...
  do k=1,npg
  wgt=xjac
  call ran1(harvest)  ! modded
  do j=1,ndim
  !call ran1(harvest)
  xn=(kg(j)-harvest(j))*dxg+1.0_wp  ! modded
